...
<template>
  ...
  <template v-else>
    <div
      class="grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8 mb-12"
    >
      <CardItem
        v-for="result in data?.results"
        :key="result.id"
        :item="result"
        :to="`/error/error-boundary/${result?.id}`"
      />
    </div>
    <NuxtLink to="/error/error-boundary/asdklashjdjklasd">
